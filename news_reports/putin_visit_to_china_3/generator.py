...
Generate the Putin visit to China news report PDF.
"""
...
def _build_requested_section(
    ...
    if request.requirement == "New Vocabulary":
        ...
        if section is None:
            section = report.sections["Comments"]
            title = request.requirement
            paragraphs = [f"Custom requested section: {request.requirement}."]
        else:
            title = section.title
            paragraphs = section.paragraphs

        story = _build_standard_section(title, paragraphs, styles)
